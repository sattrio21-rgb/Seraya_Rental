        ...
        <div class="card p-6 sticky top-24">
            ...
            @auth('web')
                @if($car->status === 'available')
                    <a href="{{ route('bookings.create', ['car_id' => $car->id]) }}" class="btn-primary w-full text-center block mb-4">
                        Booking Sekarang
                    </a>
                @else
                    <button disabled class="w-full bg-gray-300 text-gray-500 px-4 py-2 rounded-lg font-medium cursor-not-allowed mb-4">
                        Tidak Tersedia
                    </button>
                @endif
            @else
                <a href="{{ route('login') }}" class="btn-primary w-full text-center block mb-4">
                    Login untuk Booking
                </a>
            @endauth
...
@push('scripts')
<script>
    // Script kosong - tidak ada yang perlu dijalankan
</script>
@endpush
@endsection
